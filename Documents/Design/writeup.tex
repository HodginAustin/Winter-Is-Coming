...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	...
	\section{System Architecture}
		...
		\subsection{Hardware}
			...
			The hardware controls the LEDs as well as run software. This software
			...
			Using two controllers, one for LED control, and another to control timing
			and additional features. The Raspberry Pi Zero W and teensy 3.2 were chosen
			as our controllers.
			\subsubsection{Master Controller}
			The Raspberry Pi Zero W was chosen to be our master controller for its
			...
			The Raspberry Pi Zero W's ability to host a local web page while also
			...
			The Teensy 3.2 was chosen for its ability to run Arduino and C programs.
			The size of the Teensy
			as reducing the total size of the build will help cut down on minimum size
			of each box.

			The Teensy 3.2 can run Arduino code and as such can use FastLED, the LED
			control library we will be using.

			\subsubsection{LED strips}
			We will be using the WS812 Neopixel LED strips by Adafruit. These LED
...
\end{lstlisting}
		...
		\subsection{LED Control System}
			\subsubsection{Overview}
				LED control System will be handled by two different controllers. The master controller will be used to handle human input such
				...
				LEDs are clock based and can be interfered by other processes.
				\subsubsection{LED Controller Communication}
				Communication between the master controller and LED controller will be done via a serial link.
			...
			FastLED is the chosen library that will be used on the LED controller.
			FastLED, as the name implies, is a fast LED library created for Arduino projects.
			Intial code would look like This

			\begin{lstlisting}
#defin NUM_LEDS 10 // number of LEDs on the strip. use only number of LED's on the strip
